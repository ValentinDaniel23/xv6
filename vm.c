...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  void *mma = (void *)SHMEM_REGION;
  acquire(&shmem_page.lock);

  pte = walkpgdir(pgdir, mma, 0);
  ...
  if (pte && (*pte & PTE_P))
  {
    ...
    if (mappages(d, mma, PGSIZE, shmem_page.pa, PTE_W | PTE_U) < 0)
    {
      acquire(&shmem_page.lock);
      shmem_page.refcount--;
      if (shmem_page.refcount == 0)
      {
        kfree((char *)P2V(shmem_page.pa));
        shmem_page.pa = 0;
...
void *mmap()
{
  ...
  if (!shmem_page.allocated)
  {
    ...
    shmem_page.pa = V2P(page);
    shmem_page.refcount = 1;
    shmem_page.allocated = 1;
    release(&shmem_page.lock);

    if (mappages(p->pgdir, va, PGSIZE, shmem_page.pa, PTE_W | PTE_U) < 0)
    {
      acquire(&shmem_page.lock);
      shmem_page.refcount--;
      if (shmem_page.refcount == 0)
  ...
  else
  {
    ...
    if (mappages(p->pgdir, va, PGSIZE, pa, PTE_W | PTE_U) < 0)
    {
      acquire(&shmem_page.lock);
      shmem_page.refcount--;
      if (shmem_page.refcount == 0)
